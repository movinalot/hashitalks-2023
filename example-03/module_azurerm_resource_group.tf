module "module_azurerm_resource_group" {
  for_each = local.user_resource_groups_map_filtered
...
output "user_resource_groups_map_filtered" {
  value = var.enable_module_output ? local.user_resource_groups_map_filtered : null
}
